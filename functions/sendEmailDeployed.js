...
const os = require('os');
const fs = require('fs');
const sendEmailToStepOwner = require('./sendEmailHtml')
...
function sendEmail(notificationData)
{
  ...
  */
  ...
  htmlBody.then(async(htmltemplate)=>{
    ...
    await sendEmailToStepOwner.sendEmailHtml({ recipient: msgObject.to, cc: msgObject.cc, from: msgObject.from, subject: msgObject.subject, template: msgObject.html }).then(() => {
